...
export default class User {
	...
	static async getUserId(pageId: string): Promise<string | Error> {
		...
		const req = await axios.get(`https://trakteer.id/${pageId}`).catch((e) => {
			if (e.status === 404) throw new Error("User not found");
			throw new Error("Invalid page id");
		})
		const UserId = req.data.match(/(?<=creator-id=")(.*?)(?=\")/);

		if (req.status === 200 && !UserId) throw new Error("User not found/Invalid page id/Inactive page");
	...
	static async getUserDetails(userId: string): Promise<UserSummary | Error> {
		...
		const req = await axios.get<{data: UserSummary}>(`https://api.trakteer.id/v2/fe/creator/${userId}/summary`).catch((e) => {
